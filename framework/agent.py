...
5. When two candidate columns look similar, use profile_columns to distinguish proxy indicators from the correct business metric.
6. When schema columns are cryptic or code-like, use preview_rows on the most likely table to inspect example records before choosing columns.
7. Prefer normalized dimension tables over denormalized text proxy columns for business/category filtering when both are available.
8. If the metric is defined in words, thresholds, or a formula, prefer computing it from primitive fields rather than trusting a similarly named derived column.
9. Use run_sql to sanity-check the logic and output shape before you submit.
10. Call submit_answer with your final SQL query.
...
- Table grounding: I verified the fact table instead of stopping at the first plausible table.
- Column grounding: I verified the exact metric column. If similar columns exist, I compared them explicitly.
- Constraint recovery: I checked for hidden exclusions, thresholds, date rules, sentinel values, or status filters.
...
{guides_block}"""
    ...
    def _should_bootstrap_guides(self, prompt: str) -> bool:
        ...
        semantic_markers = {"only", "excluding", "exclude", "current", "active", "legacy", "official", "weighted", "comparable", "severe"}
    ...
    def _bootstrap_guide_queries(self, prompt: str) -> list[str]:
        ...
        return queries[: 1 + _AUXILIARY_BOOTSTRAP_PHRASE_LIMIT]

    def _bootstrap_catalog_queries(self, prompt: str) -> list[str]:
        queries = [prompt]
